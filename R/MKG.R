########## MKG ########################
dMKG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (!(a > 0))
    stop("MKG distribution is only defined for a > 0")
  ...
  d<-a*g*G^(a-1)*exp(-(G/(1-G))^a)/(1-G)^(a+1)
  if (log)
    d <- log(d)
  return(d)
}

pMKG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qMKG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-(-log(1-p))^(1/a)/(1+(-log(1-p))^(1/a))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rMKG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<-(-log(1-y))^(1/a)/(1+(-log(1-y))^(1/a))
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofMKG<- function (data, distr, start,gofs = "CvM")
{
  ...
  theod<-a*g*G^(a-1)*exp(-(G/(1-G))^a)/(1-G)^(a+1)
