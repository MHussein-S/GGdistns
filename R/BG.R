dBG<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0 & b > 0))
    stop("BG distribution is only defined for a > 0 and b > 0")
  ...
  d<- g * dbeta(G,shape1=a,shape2=b)
  if (log)
    d <- log(d)
  return(d)
}

pBG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qBG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-qbeta(p,shape1=a,shape2 = b)
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rBG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<-qbeta(y,shape1=a,shape2 = b)
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofBG<- function (data, distr, start,gofs = "CvM")
{
  ...
  if (gofs == "CvM")
    fnobj <- function(allpar,pdistname,data)
    {
      ...
      theop<-pbeta(G,shape1=a,shape2=b)
      n<-length(data)
  ...
  theod <- g * dbeta(G,shape1=a,shape2=b)
