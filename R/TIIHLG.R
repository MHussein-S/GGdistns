dTIIHLG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (!(a > 0))
    stop("TIIHLG distribution is only defined for a > 0")
  ...
  d<-2*a*g*G^(a-1)/(1+G^a)^2
  if (log)
    d <- log(d)
  return(d)
}

pTIIHLG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qTIIHLG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
...
mgofTIIHLG<- function (data, distr, start,gofs = "CvM")
{
