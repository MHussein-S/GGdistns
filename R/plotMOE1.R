#new version (working)
plotMOE1<-function (data, distr, para, histo = TRUE, breaks = "default", demp = FALSE, ...) 
{
...
MOPRHRF<-alpha*ypdf/(ycdf*(1-alpha_bar*(1-ycdf)))
yhist <- MOPDF
...
ycdf <- do.call(pdistname, c(list(obsp), as.list(para)))
...
theop <- MOCDF_s
	...
	stop("problem when computing probabilities.")
plot(theop, obsp, main = "P-P plot", xlab = "Theoretical probabilities", ylab = "Empirical probabilities", ...)
abline(0, 1)	
}
par(def.par)
invisible()
