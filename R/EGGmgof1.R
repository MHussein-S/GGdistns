#Maximum goodness-of-fit estimates of Exponentiated Generalized G distribution "using constroptim"
# start is must be a named vector ex: EEPGmgof1(data, distr="weibull", gof = "AD", start=c(a=0.5,b=0.5,scale=0.5,shape=0.5))
EGGmgof1 <- function (data, distr, start,gof = "CvM",...)
{
  ...
  if (is.na(bpar))
    stop("Starting value of \"b\" is not defined")
  b<-start["b"]
  par<-par[-bpar]
  #--------
  ...
  if (gof == "CvM")
    fnobj <- function(allpar,pdistnam,data)
    {
      ...
      a<-allpar$a
      distpar<-allpar[-apar]
      bpar<-match("b",names(distpar))
      b<-allpar$b
      distpar<-distpar[-bpar]
  ...
  theod<-a*b*g*(1-G)^(a-1)*(1-(1-G)^a)^(b-1)
