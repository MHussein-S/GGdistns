#Maximum goodness-of-fit estimates of Modified Kies G distribution "using constroptim"
# start is must be a named vector ex: MKGmgof1(data, distr="weibull", gof = "AD", start=c(a=0.5,scale=0.5,shape=0.5))
MKGmgof1 <- function (data, distr, start,gof = "CvM",...)
{
  if (!(is.numeric(data) & length(data)>1))
    stop("data must be a numeric vector of length greater than 1")
  if(is.list(start)|is.null(names(start)))
    stop("'start' must be a named atomic vector of the form 'c(name=val,name=val,...)'")
