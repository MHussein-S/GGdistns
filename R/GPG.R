########## GPG ########################
dGPG<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0) | !(b > 0 & b < 1))
    stop("GPG distribution is only defined for a > 0 and 0 < b < 1")
  ...
  d<-a*(1-b)*(1-exp(-a))*g*exp(-a+a*G)/(1-exp(-a)-b+b*exp(-a+a*G))^2
  if (log)
    d <- log(d)
  return(d)
}

pGPG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
  ...
  p<-(exp(-a+a*G)-exp(-a))/(1-exp(-a)-b+b*exp(-a+a*G))
  if (!lower.tail)
    p <- 1 - p
  if (log.p)
    p <- log(p)
...
qGPG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<- 1/a*log(((1-exp(-a)-b)*p+exp(-a))/((1-b*p)*exp(-a)))
  q<- do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rGPG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<- 1/a*log(((1-exp(-a)-b)*y+exp(-a))/((1-b*y)*exp(-a)))
...
mgofGPG<- function (data, distr, start,gofs = "CvM")
{
  ...
  if (gofs == "CvM")
    fnobj <- function(allpar,pdistname,data)
    {
      ...
      theop<-(exp(-a+a*G)-exp(-a))/(1-exp(-a)-b+b*exp(-a+a*G))
      n<-length(data)
  ...
  distpar<-parset$distpar
  opt <- constrOptim(theta=start,f=fnobj,pdistname=pdistname, data = data ,grad=NULL, ui=Mat,ci = c(0,-1,rep(0,length(distpar))))
  #---------------
  ...
  theod<-a*(1-b)*(1-exp(-a))*g*exp(-a+a*G)/(1-exp(-a)-b+b*exp(-a+a*G))^2
