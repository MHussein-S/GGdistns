########## LGGII ########################
dLGGII<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0 & b > 0))
    stop("LGGII distribution is only defined for a > 0 and b > 0")
  ...
  d<-dgamma(-b*log(G),shape=a)*b*g/G
  if (log)
    d <- log(d)
  return(d)
}

pLGGII<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qLGGII<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-exp(-1/b*qgamma(1-p,shape = a))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rLGGII<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
...
mgofLGGII<- function (data, distr, start,gofs = "CvM")
{
  ...
  if (gofs == "CvM")
    fnobj <- function(allpar,pdistname,data)
    {
      ...
      theop<-1-pgamma(-b*log(G),shape = a)
      n<-length(data)
  ...
  theod<-dgamma(-b*log(G),shape=a)*b*g/G
