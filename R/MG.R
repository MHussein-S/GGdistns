########## MG ########################
dMG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (a > 1 | a == 0)
    stop("MG distribution is not defined for a = 0 and a > 1")
  ...
  d<- exp(1/a)*g*(1-a*G^a)*G^(-2*a-1)*exp(-1/a*G^-a)
  if (log)
    d <- log(d)
  return(d)
}

pMG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qMG<-function(p,par,distr,lower=0,upper,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
rMG<-function(n,par,distr)
{
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  qMG(y, par=par,distr=distr,upper=10000)
}

mgofMG<- function (data, distr, start,gofs = "CvM")
{
  ...
  distpar<-parset$distpar
  opt <- constrOptim(theta=start,f=fnobj,pdistname=pdistname, data = data ,grad=NULL, ui=Mat,ci = c(-1,rep(0,length(distpar))))
  #---------------
  ...
  theod<-exp(1/a)*g*(1-a*G^a)*G^(-2*a-1)*exp(-1/a*G^-a)
