...
CDF<-do.call(pdistname, c(list(x), as.list(par1)))
y<--log(1-CDF)
#alpha<-optim(par=alpha0, fn=objfn , obs=y)
alpha<-optim(par=alpha0, fn=objfn , obs=y,method = "Brent")
start <- c(tilt=alpha$par,par1)
return(start)
} 
# ------------------2- MOEmanageparam---------------------
