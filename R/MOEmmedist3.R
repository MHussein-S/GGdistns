##using new version of MOEstart ----- working good
#using the first transformation calculating the rth moment
...
m <- match(parnm,args)
	...
	{#integrand 
	alpha_bar<-1-alpha
x<-(1-alpha*z)/alpha_bar*z
	L1<-do.call(qdistname, c(list(x), as.list(par)))
	L1<-L1^order
	}#integrand 
a<-integrate(integrand, lower = 1, upper =1/alpha,alpha=alpha,qdistname=qdistname ,par=par,order=order)
a<-(alpha/alpha_bar )*a$value
return(a)
}#mMOE
