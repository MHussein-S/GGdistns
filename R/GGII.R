########## GGII ########################
dGGII<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0 & b > 0))
    stop("GGII distribution is only defined for a > 0 and b > 0")
  ...
  d<-dgamma(b*G/(1-G),shape = a)*b*g/(1-G)^2
  if (log)
    d <- log(d)
  return(d)
}

pGGII<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qGGII<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-qgamma(p,shape = a)/(b+qgamma(p,shape = a))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rGGII<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y <- runif(n)
  y <- qgamma(y,shape = a)/(b+qgamma(y,shape = a))
...
mgofGGII<- function (data, distr, start,gofs = "CvM")
{
  ...
  theod<-dgamma(b*G/(1-G),shape = a)*b*g/(1-G)^2
