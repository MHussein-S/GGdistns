dNMEPAG<-function (x,par,distr, log = FALSE)
{
  ...
  d<-g*(1-2*a*G*(1-a*G))/exp((1-a)^2-(1-a*G)^2)
  if (log)
    d <- log(d)
  return(d)
}

pNMEPAG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
mgofNMEPAG<- function (data, distr, start,gofs = "CvM")
{
  ...
  if (gofs == "CvM")
    ...
    if (gofs == "AD2R")
      fnobj <- function(allpar,pdistname,data)
      {
        ...
        distpar<-parset$distpar
        G<-do.call(pdistname,c(list(data),as.list(distpar)))
        theop<-g*(1-2*a*G*(1-a*G))/exp((1-a)^2-(1-a*G)^2)
        n<-length(data)
  ...
  theod<-g*(1-2*a*G*(1-a*G))/exp((1-a)^2-(1-a*G)^2)
