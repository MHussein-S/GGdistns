dPG<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0 & b > 0))
    stop("PG distribution is only defined for a > 0 and b > 0")
  ...
  d<- a * b * (1-exp(-a))^-1 * g * G^(b-1) * exp(-a*G^b)
  if (log)
    d <- log(d)
  return(d)
}

pPG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qPG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<- ((-1/a)*log(1-p*(1-exp(-a))))^(1/b)
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rPG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<- ((-1/a)*log(1-y*(1-exp(-a))))^(1/b)
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofPG<- function (data, distr, start,gofs = "CvM")
{
  ...
  theod<-a * b * (1-exp(-a))^-1 * g * G^(b-1) * exp(-a*G^b)
