########## MBG ########################
dMBG<-function (x,par,distr, log = FALSE)
{
  ...
  if(is.na(apar)|is.na(bpar)|is.na(cpar))
    stop("at least one of the extra parameters '(a,b,c)' are not  specified")
  args <- names(formals(ddistname))
  a<-par$a
  b<-par$b
  c<-par$c
  if (!(a > 0 & b > 0 & c > 0))
    stop("MBG distribution is only defined for a,b,c > 0")
  distparn<-setdiff(names(par),c("a","b","c"))
  ...
  d<-c^a*g/(1-(1-c)*G)^(a+b)*dbeta(G,shape1=a,shape2=b)
  if (log)
    d <- log(d)
  return(d)
}

pMBG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
  ...
  p<-pbeta(c*G/(1+(c-1)*G),shape1=a,shape2=b)
  if (!lower.tail)
    p <- 1 - p
  if (log.p)
    p <- log(p)
...
qMBG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-qbeta(p,shape1=a,shape2 = b)/(c-(c-1)*qbeta(p,shape1=a,shape2 = b))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rMBG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<-qbeta(y,shape1 = a,shape2 = b)/(c-(c-1)*qbeta(y,shape1 = a,shape2 = b))
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofMBG<- function (data, distr, start,gofs = "CvM")
{
  ...
  if (gofs == "CvM")
    fnobj <- function(allpar,pdistname,data)
    {
      ...
      theop<-pbeta(c*G/(1+(c-1)*G),shape1=a,shape2=b)
      n<-length(data)
  ...
  theod<-c^a*g/(1-(1-c)*G)^(a+b)*dbeta(G,shape1=a,shape2=b)
