dMPoG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (a < exp(-1))
    stop("MPoG distribution is not defined for a < exp(-1)")
...
mgofMPoG<- function (data, distr, start,gofs = "CvM")
{
  ...
  distpar<-parset$distpar
  opt <- constrOptim(theta=start,f=fnobj,pdistname=pdistname, data = data ,grad=NULL, ui=Mat,ci = c(exp(-1),rep(0,length(distpar))))
  #---------------
