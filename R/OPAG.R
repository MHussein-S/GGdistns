dOPAG<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0 & b > 0))
    stop("OPAG distribution is only defined for a,b > 0")
  ...
  d<-a*b*g/G^2*((1-G)/(1-(1-b)*G))^(a+1)
  if (log)
    d <- log(d)
  return(d)
}

pOPAG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qOPAG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-(1-(1-p)^(1/a))/(1-(1-b)*(1-p)^(1/a))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rOPAG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
...
mgofOPAG<- function (data, distr, start,gofs = "CvM")
{
  ...
  theod<-a*b*g/G^2*((1-G)/(1-(1-b)*G))^(a+1)
