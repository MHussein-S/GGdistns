#Maximum goodness-of-fit estimates of Exponentiated Exponential Poisson G distribution
EEPGmgof <- function (data, distr, start,gof = "CvM",...)
{
  start_rev<-start
 ...
 # colnames(Mat) <- c("alpha","beta","rate")
  ...
  initconstr <- Mat %*% start_rev - seq(0,0,length(start))
  if (any(initconstr < 0))
    stop("Starting values must be in the feasible region.")
  opt <- constrOptim(theta=start_rev,f=fnobj,pdistnam=pdistname, data = data ,grad=NULL, ui=Mat,ci = seq(0,0,length(start)))
  #---------------
