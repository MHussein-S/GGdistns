dEGG<-function (x,par,distr, log = FALSE)
{
  ...
  b<-par$b
  if (!(a > 0 & b > 0))
    stop("EGG distribution is only defined for a > 0 and b > 0")
  ...
  d<- a * b * g * (1 - G)^(a-1) * (1 - (1 - G) ^ a) ^ (b-1)
  if (log)
    d <- log(d)
  return(d)
}

pEGG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qEGG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
...
mgofEGG<- function (data, distr, start,gofs = "CvM")
{
