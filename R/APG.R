...
dAPG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (a < 0 | a == 1)
    stop("APG distribution is not defined for a = 1 and a < 0")
...
pAPG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
