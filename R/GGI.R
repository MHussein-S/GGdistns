########## GGI ########################
dGGI<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (!(a > 0))
    stop("GGI distribution is only defined for a > 0")
  ...
  d<-dgamma(-log(1-G),shape = a)*g/(1-G)
  if (log)
    d <- log(d)
  return(d)
}

pGGI<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qGGI<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-1-exp(-qgamma(p,shape = a))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rGGI<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<-1-exp(-qgamma(y,shape = a))
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofGGI<- function (data, distr, start,gofs = "CvM")
{
  ...
  theod<-dgamma(-log(1-G),shape = a)*g/(1-G)
