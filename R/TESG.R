########## TESG ########################
dTESG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (!(a > 0))
    stop("TESG distribution is only defined for a > 0")
  ...
  d<-a*g*exp(-a*G)/(1-exp(-a))
  if (log)
    d <- log(d)
  return(d)
}

pTESG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qTESG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<--1/a*log(1-p*(1-exp(-a)))
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rTESG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<--1/a*log(1-y*(1-exp(-a)))
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofTESG<- function (data, distr, start,gofs = "CvM")
{
