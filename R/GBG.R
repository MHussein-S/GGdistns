########## GBG ########################
dGBG<-function (x,par,distr, log = FALSE)
{
  ...
  c<-par$c
  if (!(a > 0 & b > 0 & c > 0))
    stop("GBG distribution is only defined for a,b,c > 0")
  ...
  d<-c*g*G^(c-1)*dbeta(G^c,shape1=a,shape2=b)
  if (log)
    d <- log(d)
  return(d)
}

pGBG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
  ...
  c<-par$c
  if (!(a > 0 & b > 0 & c > 0))
    stop("GBG distribution is only defined for a,b > 0,c > 0")
  distparn<-setdiff(names(par),c("a","b","c"))
  distpar<-par[distparn]
  m <- match(distparn,args)
  if (any(is.na(m)))
...
qGBG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-qbeta(p,shape1=a,shape2 = b)^(1/c)
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rGBG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
  ...
  if (any(is.na(m)))
    stop("you specified invalid names of parameters for ",distr)
  y<-runif(n)
  y<-qbeta(y,shape1=a,shape2 = b)^(1/c)
  do.call(qdistname,c(list(y), as.list(distpar)))
}

mgofGBG<- function (data, distr, start,gofs = "CvM")
{
  ...
  if (gofs == "CvM")
    fnobj <- function(allpar,pdistname,data)
    {
      ...
      theop<-pbeta(G^c,shape1=a,shape2=b)
      n<-length(data)
  ...
  theod<-c*g*G^(c-1)*dbeta(G^c,shape1=a,shape2=b)
