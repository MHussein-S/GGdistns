dTIHTG<-function (x,par,distr, log = FALSE)
{
  ...
  a<-par$a
  if (!(a > 0))
    stop("TIHTG distribution is only defined for a > 0")
  ...
  d<-a^2*g*(1-G)^(a-1)/(1-(1-a)*G)^(a+1)
  if (log)
    d <- log(d)
  return(d)
}

pTIHTG<-function (q,par,distr, lower.tail = TRUE, log.p = FALSE )
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  pdistname <- paste("p", distr, sep = "")
...
qTIHTG<-function(p,par,distr,lower.tail=TRUE,log.p=FALSE)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  ...
  if (lower.tail==FALSE)
    p<-1-p
  p<-((1-p)^(1/a)-1)/((1-a)*(1-p)^(1/a)-1)
  q<-do.call(qdistname,c(list(p), as.list(distpar)))
  return(q)
}
rTIHTG<-function(n,par,distr)
{
  if(!is.list(par)|is.null(names(par)))
    stop("'par' must be a named list")
  qdistname <- paste("q", distr, sep = "")
...
mgofTIHTG<- function (data, distr, start,gofs = "CvM")
{
  ...
  theod<-a^2*g*(1-G)^(a-1)/(1-(1-a)*G)^(a+1)
